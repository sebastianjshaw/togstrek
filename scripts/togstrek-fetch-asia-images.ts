/**
 * Download Squarespace CDN images referenced in Asia place MDX under
 * `content/places/asia/` (recursive) into
 * `migration/cdn-upload-ready/asia/{country}/{place}/…` and rewrite MDX to
 * `https://media.togstrek.com/asia/{country}/{place}/…/{file}`.
 *
 * Run after `npm run migrate:places:asia`. Then upload:
 *   npm run r2:upload -- --source migration/cdn-upload-ready/asia --prefix asia/
 *
 * Usage:
 *   npm run media:fetch-asia -- --dry-run
 *   npm run media:fetch-asia
 *   npm run media:fetch-asia -- --limit 3
 */
...
const PLACES_ASIA = path.join(REPO_ROOT, "content", "places", "asia");
...
const MEDIA_PUBLIC_BASE =
  process.env.TOGSTREK_MEDIA_PUBLIC_BASE_ASIA?.trim() ||
  "https://media.togstrek.com/asia";
...
function listAsiaMdxFiles(): string[] {
  const out: string[] = [];
...
/** `.../asia/<country>/….mdx` (any nesting under country) → country + place path segments. */
function countryPlaceFromMdxPath(mdxAbs: string): {
  country: string;
  placeSegments: string[];
} | null {
  const rel = path.relative(PLACES_ASIA, mdxAbs).replace(/\\/g, "/");
...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(PLACES_ASIA)) {
    console.error(`Missing ${PLACES_ASIA}`);
    process.exit(1);
  }

  let files = listAsiaMdxFiles();
  if (limit !== null) files = files.slice(0, limit);

  console.log(
    `Asia image fetch: ${files.length} MDX files → ${path.relative(REPO_ROOT, outRoot)}`,
  );
  ...
  if (!dryRun && totalFail === 0) {
    console.log(
      `\nUpload: npm run r2:upload -- --source migration/cdn-upload-ready/asia --prefix asia/`,
    );
  }

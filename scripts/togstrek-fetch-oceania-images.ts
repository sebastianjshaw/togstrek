/**
 * Download Squarespace CDN images referenced in Oceania place MDX under
 * `content/places/oceania/` (recursive) into
 * `migration/cdn-upload-ready/oceania/{country}/{place}/…` and rewrite MDX to
 * `https://media.togstrek.com/oceania/{country}/{place}/…/{file}`.
 *
 * Run after `npm run migrate:places:oceania`. Then upload:
 *   npm run r2:upload -- --source migration/cdn-upload-ready/oceania --prefix oceania/
 *
 * Usage:
 *   npm run media:fetch-oceania -- --dry-run
 *   npm run media:fetch-oceania
 *   npm run media:fetch-oceania -- --limit 3
 */
...
const PLACES_OCEANIA = path.join(REPO_ROOT, "content", "places", "oceania");
...
const MEDIA_PUBLIC_BASE =
  process.env.TOGSTREK_MEDIA_PUBLIC_BASE_OCEANIA?.trim() ||
  "https://media.togstrek.com/oceania";
...
function listOceaniaMdxFiles(): string[] {
  const out: string[] = [];
...
/** `.../oceania/<country>/….mdx` (any nesting under country) → country + place path segments. */
function countryPlaceFromMdxPath(mdxAbs: string): {
  country: string;
  placeSegments: string[];
} | null {
  const rel = path.relative(PLACES_OCEANIA, mdxAbs).replace(/\\/g, "/");
...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(PLACES_OCEANIA)) {
    console.error(`Missing ${PLACES_OCEANIA}`);
    process.exit(1);
  }

  let files = listOceaniaMdxFiles();
  if (limit !== null) files = files.slice(0, limit);

  console.log(
    `Oceania image fetch: ${files.length} MDX files → ${path.relative(REPO_ROOT, outRoot)}`,
  );
  ...
  if (!dryRun && totalFail === 0) {
    console.log(
      `\nUpload: npm run r2:upload -- --source migration/cdn-upload-ready/oceania --prefix oceania/`,
    );
  }

/**
 * Download Squarespace CDN images referenced in Africa place MDX under
 * `content/places/africa/` (recursive) into
 * `migration/cdn-upload-ready/africa/{country}/{place}/…` and rewrite MDX to
 * `https://media.togstrek.com/africa/{country}/{place}/…/{file}`.
 *
 * Run after `npm run migrate:places:africa`. Then upload:
 *   npm run r2:upload -- --source migration/cdn-upload-ready/africa --prefix africa/
 *
 * Usage:
 *   npm run media:fetch-africa -- --dry-run
 *   npm run media:fetch-africa
 *   npm run media:fetch-africa -- --limit 3
 */
...
const PLACES_AFRICA = path.join(REPO_ROOT, "content", "places", "africa");
...
const MEDIA_PUBLIC_BASE =
  process.env.TOGSTREK_MEDIA_PUBLIC_BASE_AFRICA?.trim() ||
  "https://media.togstrek.com/africa";
...
function listAfricaMdxFiles(): string[] {
  const out: string[] = [];
...
/** `.../africa/<country>/….mdx` (any nesting under country) → country + place path segments. */
function countryPlaceFromMdxPath(mdxAbs: string): {
  country: string;
  placeSegments: string[];
} | null {
  const rel = path.relative(PLACES_AFRICA, mdxAbs).replace(/\\/g, "/");
...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(PLACES_AFRICA)) {
    console.error(`Missing ${PLACES_AFRICA}`);
    process.exit(1);
  }

  let files = listAfricaMdxFiles();
  if (limit !== null) files = files.slice(0, limit);

  console.log(
    `Africa image fetch: ${files.length} MDX files → ${path.relative(REPO_ROOT, outRoot)}`,
  );
  ...
  if (!dryRun && totalFail === 0) {
    console.log(
      `\nUpload: npm run r2:upload -- --source migration/cdn-upload-ready/africa --prefix africa/`,
    );
  }

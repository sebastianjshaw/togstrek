#!/usr/bin/env python3
"""
Server-side copy within R2: mirror media to canonical key prefixes
  europe/turkiye/said/   → europe/turkiye/side/
  europe/lichtenstein/   → europe/liechtenstein/

Same R2 creds as r2_upload.py. Skips if destination key already exists
unless --no-skip-existing.
"""
...
def main() -> int:
    ...
    if args.dry_run:
        run_copy(
            s3, bucket, "europe/turkiye/said/", "europe/turkiye/side/", True, False
        )
        run_copy(
            s3,
            bucket,
    ...
    a, s, f = run_copy(
        s3, bucket, "europe/turkiye/said/", "europe/turkiye/side/", False, skip
    )
    print(f"side: ok={a} skip={s} err={f}", flush=True)
    a2, s2, f2 = run_copy(
        s3, bucket, "europe/lichtenstein/", "europe/liechtenstein/", False, skip
    )
    print(f"liechtenstein: ok={a2} skip={s2} err={f2}", flush=True)
    return 0 if (f + f2) == 0 else 1

/**
 * Migrate Squarespace HTTrack Europe place HTML to
 * `content/places/europe/<country>/…/<place>.mdx` (nested paths when the mirror
 * has subfolders, e.g. `united-kingdom/england/greater-london/london.html`) and copy images to
 * `migration/cdn-upload-ready/europe/...` for CDN upload
 * (public URLs: `https://media.togstrek.com/europe/...` — same pattern as North America).
 *
 * Backup layout: `denmark/copenhagen.html`, root `belgrade.html` (→ `serbia/belgrade.mdx`),
 * duplicate `europe/portugal/...` under the continent tree is normalized, mirror typos
 * (`garmany` → austria, `engand` → england) corrected. Skips `tag/`, `category/`, `_https_`,
 * and `template-*.html`.
 *
 * Usage:
 *   npx tsx scripts/migrate-squarespace-places-europe.ts --dry-run
 *   npx tsx scripts/migrate-squarespace-places-europe.ts --backup /path/to/TogsTrekBackup
 *   npx tsx scripts/migrate-squarespace-places-europe.ts --limit 3
 *
 * CDN (Cloudflare R2) after migrate:
 *   pip install -r scripts/requirements-r2.txt
 *   export R2_ACCOUNT_ID=… R2_BUCKET_NAME=… R2_ACCESS_KEY_ID=… R2_SECRET_ACCESS_KEY=…
 *   npm run r2:upload -- --source migration/cdn-upload-ready/europe --prefix europe/
 *   (keys become europe/denmark/copenhagen/… matching MDX URLs.)
 */
...
/** CDN paths mirror site URLs: /europe/denmark/copenhagen/photo.jpg */
...
function migrateOnePlacePage(args: {
  ...
  if (args.slugSegments.length < 3 || args.slugSegments[0] !== "europe") {
    console.warn(
      `Skip (expected europe/<country>/…): ${args.slugSegments.join("/")}`,
    );
    return;
  }
...
const EUROPE_SKIP_DIRS = new Set(["tag", "category", "_https_"]);

/**
 * Swedish län folders mirrored at `europe/<län>/…` (sibling to `europe/sweden/…`).
 * Nest under `sweden/<län>/…` so country hubs and travel data align with Sweden (ISO SE).
 */
const EUROPE_ROOT_SWEDISH_LAN_SLUGS = new Set([
  "gavleborg",
  "orebro",
  "ostergotland",
  "vasterbotten",
...
/** Root `europe/<place>.html` (no country folder) → canonical country slug for URLs/MDX. */
const EUROPE_ROOT_PLACE_TO_COUNTRY: Record<string, string> = {
  belgrade: "serbia",
  brans: "sweden",
  crete: "greece",
  helsinki: "finland",
...
function resetEuropeStaging(cdnOut: string, dryRun: boolean): void {
  const target = path.join(cdnOut, "europe");
  if (dryRun || !fs.existsSync(target)) return;
  fs.rmSync(target, { recursive: true });
}

/** Fix duplicate continent segment and known HTTrack folder typos (paths relative to `europe/`). */
function normalizeEuropeMirrorParts(parts: string[]): string[] {
  let out = [...parts];
  if (out[0] === "europe" && out.length >= 2) {
    out.shift();
  }
  if (out[0] === "garmany") {
    out[0] = "austria";
  }
  if (out[0] === "united-kingdom" && out[1] === "engand") {
    out[1] = "england";
  }
  if (out[0] && EUROPE_ROOT_SWEDISH_LAN_SLUGS.has(out[0]!)) {
    out = ["sweden", ...out];
  }
  if (out[0] === "sweden" && out[1] === "vasterbotton") {
    out[1] = "vasterbotten";
  }
  return out;
}

/** All place `.html` files under `europe/` (recursive), excluding tag/category and templates. */
function listEuropePlaceHtmlFiles(europeDir: string): string[] {
  const out: string[] = [];
  function walk(dir: string): void {
    ...
    for (const ent of entries) {
      const full = path.join(dir, ent.name);
      if (ent.isDirectory()) {
        if (EUROPE_SKIP_DIRS.has(ent.name)) continue;
        walk(full);
      } else if (
        ent.isFile() &&
        ent.name.endsWith(".html") &&
        !ent.name.includes("_https_") &&
        !ent.name.toLowerCase().startsWith("template-")
      ) {
        out.push(full);
      }
    }
...
/**
 * Map backup file under `togstrek.com/europe/` to full URL segments
 * `europe/<country>/…/<place>` (same nesting rules as `content/places/europe/`).
 */
function htmlPathToEuropeSlugSegments(
  europeDir: string,
  htmlAbs: string,
): string[] | null {
  const rel = path.relative(europeDir, htmlAbs).replace(/\\/g, "/");
  const without = rel.replace(/\.html$/i, "");
  const rawParts = without.split("/").filter(Boolean);
  if (rawParts.length === 0) return null;

  const leaf = rawParts[rawParts.length - 1]!;
  if (leaf.toLowerCase().startsWith("template-")) return null;

  const parts = normalizeEuropeMirrorParts(rawParts);

  if (parts.length === 1) {
    const place = parts[0]!;
    const country = EUROPE_ROOT_PLACE_TO_COUNTRY[place];
    if (!country) return null;
    return ["europe", country, place];
  }
...
function main(): void {
  ...
  const europeDir = path.join(togstrekCom, "europe");
  const contentRoot = path.join(REPO_ROOT, "content");
  const cdnOut = path.join(REPO_ROOT, "migration", "cdn-upload-ready");

  if (!fs.existsSync(europeDir)) {
    console.error(`Europe dir missing: ${europeDir}`);
    process.exit(1);
  }
  ...
  resetEuropeStaging(cdnOut, dryRun);
  if (!dryRun) {
    console.log(
      "Cleared migration/cdn-upload-ready/europe for a clean place image tree.",
    );
  }

  let postHtml = listEuropePlaceHtmlFiles(europeDir);
  if (limit !== null) postHtml = postHtml.slice(0, limit);

  console.log(
    `Migrating ${postHtml.length} Europe place pages${limit ? ` (limit ${limit})` : ""}.`,
  );

  for (const htmlAbs of postHtml) {
    const slugSegments = htmlPathToEuropeSlugSegments(europeDir, htmlAbs);
    if (!slugSegments || slugSegments[0] !== "europe") {
      console.warn(
        `Skip unexpected path: ${posix(path.relative(europeDir, htmlAbs))}`,
      );
      continue;
    }
    migrateOnePlacePage({
  ...
  if (dryRun) {
    ...
    console.log("Done.");
    console.log(
      `Upload to CDN: npm run r2:upload -- --source migration/cdn-upload-ready/europe --prefix europe/`,
    );
    console.log(pathToFileURL(path.join(cdnOut, "europe")).href);
  }
}

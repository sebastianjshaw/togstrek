/**
 * Migrate Squarespace HTTrack Africa place HTML to
 * `content/places/africa/<country>/…/<place>.mdx` (nested paths when the mirror
 * has subfolders, e.g. `tanzania/zanzibar/stone-town.html`) and copy images to
 * `migration/cdn-upload-ready/africa/...` for CDN upload
 * (public URLs: `https://media.togstrek.com/africa/...`).
 *
 * Backup layout: `togstrek.com/africa/<country>/<place>.html` (recursive).
 * Skips `tag/`, `category/`, `_https_`, and `template-*.html`.
 *
 * Usage:
 *   npx tsx scripts/migrate-squarespace-places-africa.ts --dry-run
 *   npx tsx scripts/migrate-squarespace-places-africa.ts --backup /path/to/TogsTrekBackup
 *   npx tsx scripts/migrate-squarespace-places-africa.ts --limit 3
 *
 ...
 *   npm run r2:upload -- --source migration/cdn-upload-ready/africa --prefix africa/
 *   (keys become africa/kenya/nairobi/… matching MDX URLs.)
 *
 * If the HTTrack backup has almost no image binaries (common), run after migrate:
 *   npm run media:fetch-africa
 * That downloads Squarespace CDN URLs from the new MDX, writes files under
 * `migration/cdn-upload-ready/africa/`, and rewrites MDX to `media.togstrek.com` paths.
 */
...
/** CDN paths mirror site URLs: /africa/kenya/nairobi/photo.jpg */
...
function migrateOnePlacePage(args: {
  ...
  if (args.slugSegments.length < 3 || args.slugSegments[0] !== "africa") {
    console.warn(
      `Skip (expected africa/<country>/…): ${args.slugSegments.join("/")}`,
    );
    return;
  }
...
const AFRICA_SKIP_DIRS = new Set(["tag", "category", "_https_"]);

function resetAfricaStaging(cdnOut: string, dryRun: boolean): void {
  const target = path.join(cdnOut, "africa");
  if (dryRun || !fs.existsSync(target)) return;
  fs.rmSync(target, { recursive: true });
}

/** All place `.html` files under `africa/` (recursive), excluding tag/category and templates. */
function listAfricaPlaceHtmlFiles(africaDir: string): string[] {
  const out: string[] = [];
  function walk(dir: string): void {
    ...
    for (const ent of entries) {
      const full = path.join(dir, ent.name);
      if (ent.isDirectory()) {
        if (AFRICA_SKIP_DIRS.has(ent.name)) continue;
        walk(full);
      } else if (
        ent.isFile() &&
...
/**
 * Map backup file under `togstrek.com/africa/` to full URL segments
 * `africa/<country>/…/<place>` (e.g. nested `tanzania/zanzibar/stone-town`).
 */
function htmlPathToAfricaSlugSegments(
  africaDir: string,
  htmlAbs: string,
): string[] | null {
  const rel = path.relative(africaDir, htmlAbs).replace(/\\/g, "/");
...
function main(): void {
  ...
  resetAfricaStaging(cdnOut, dryRun);
  if (!dryRun) {
    console.log(
      "Cleared migration/cdn-upload-ready/africa for a clean place image tree.",
    );
  }

  let postHtml = listAfricaPlaceHtmlFiles(africaDir);
  if (limit !== null) postHtml = postHtml.slice(0, limit);

  console.log(
    `Migrating ${postHtml.length} Africa place pages${limit ? ` (limit ${limit})` : ""}.`,
  );

  for (const htmlAbs of postHtml) {
    const slugSegments = htmlPathToAfricaSlugSegments(africaDir, htmlAbs);
    if (!slugSegments || slugSegments[0] !== "africa") {
      console.warn(
        `Skip unexpected path: ${posix(path.relative(africaDir, htmlAbs))}`,
      );
      continue;
    }
    migrateOnePlacePage({
  ...
  if (dryRun) {
    ...
    console.log("Done.");
    console.log(
      `Upload to CDN: npm run r2:upload -- --source migration/cdn-upload-ready/africa --prefix africa/`,
    );
    console.log(pathToFileURL(path.join(cdnOut, "africa")).href);
  }
}

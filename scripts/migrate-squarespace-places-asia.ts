/**
 * Migrate Squarespace HTTrack Asia place HTML to
 * `content/places/asia/<country>/…/<place>.mdx` (nested paths when the mirror
 * has subfolders, e.g. `thailand/ko-phi-phi.html`) and copy images to
 * `migration/cdn-upload-ready/asia/...` for CDN upload
 * (public URLs: `https://media.togstrek.com/asia/...`).
 *
 * Backup layout: `togstrek.com/asia/<country>/<place>.html` (recursive).
 * Skips `tag/`, `category/`, `_https_`, and `template-*.html`.
 *
 * Usage:
 *   npx tsx scripts/migrate-squarespace-places-asia.ts --dry-run
 *   npx tsx scripts/migrate-squarespace-places-asia.ts --backup /path/to/TogsTrekBackup
 *   npx tsx scripts/migrate-squarespace-places-asia.ts --limit 3
 *
 ...
 *   npm run r2:upload -- --source migration/cdn-upload-ready/asia --prefix asia/
 *   (keys become asia/jordan/petra/… matching MDX URLs.)
 *
 * If the HTTrack backup has almost no image binaries (common), run after migrate:
 *   npm run media:fetch-asia
 * That downloads Squarespace CDN URLs from the new MDX, writes files under
 * `migration/cdn-upload-ready/asia/`, and rewrites MDX to `media.togstrek.com` paths.
 */
...
/** CDN paths mirror site URLs: /asia/jordan/petra/photo.jpg */
...
function migrateOnePlacePage(args: {
  ...
  if (args.slugSegments.length < 3 || args.slugSegments[0] !== "asia") {
    console.warn(
      `Skip (expected asia/<country>/…): ${args.slugSegments.join("/")}`,
    );
    return;
  }
...
const ASIA_SKIP_DIRS = new Set(["tag", "category", "_https_"]);

function resetAsiaStaging(cdnOut: string, dryRun: boolean): void {
  const target = path.join(cdnOut, "asia");
  if (dryRun || !fs.existsSync(target)) return;
  fs.rmSync(target, { recursive: true });
}

/** All place `.html` files under `asia/` (recursive), excluding tag/category and templates. */
function listAsiaPlaceHtmlFiles(asiaDir: string): string[] {
  const out: string[] = [];
  function walk(dir: string): void {
    ...
    for (const ent of entries) {
      const full = path.join(dir, ent.name);
      if (ent.isDirectory()) {
        if (ASIA_SKIP_DIRS.has(ent.name)) continue;
        walk(full);
      } else if (
        ent.isFile() &&
...
/**
 * Map backup file under `togstrek.com/asia/` to full URL segments
 * `asia/<country>/…/<place>` (e.g. nested `thailand/ko-phi-phi`).
 */
/** HTTrack folder typos under `asia/` (first path segment = country slug). */
function normalizeAsiaMirrorParts(parts: string[]): string[] {
  const out = [...parts];
  if (out[0] === "isreal") out[0] = "israel";
  if (out[0] === "turkiye") out[0] = "turkey";
  return out;
}

function htmlPathToAsiaSlugSegments(
  asiaDir: string,
  htmlAbs: string,
): string[] | null {
  const rel = path.relative(asiaDir, htmlAbs).replace(/\\/g, "/");
  ...
  const parts = normalizeAsiaMirrorParts(rawParts);
  return ["asia", ...parts];
}
...
function main(): void {
  ...
  resetAsiaStaging(cdnOut, dryRun);
  if (!dryRun) {
    console.log(
      "Cleared migration/cdn-upload-ready/asia for a clean place image tree.",
    );
  }

  let postHtml = listAsiaPlaceHtmlFiles(asiaDir);
  if (limit !== null) postHtml = postHtml.slice(0, limit);

  console.log(
    `Migrating ${postHtml.length} Asia place pages${limit ? ` (limit ${limit})` : ""}.`,
  );

  for (const htmlAbs of postHtml) {
    const slugSegments = htmlPathToAsiaSlugSegments(asiaDir, htmlAbs);
    if (!slugSegments || slugSegments[0] !== "asia") {
      console.warn(
        `Skip unexpected path: ${posix(path.relative(asiaDir, htmlAbs))}`,
      );
      continue;
    }
    migrateOnePlacePage({
  ...
  if (dryRun) {
    ...
    console.log("Done.");
    console.log(
      `Upload to CDN: npm run r2:upload -- --source migration/cdn-upload-ready/asia --prefix asia/`,
    );
    console.log(pathToFileURL(path.join(cdnOut, "asia")).href);
  }
}

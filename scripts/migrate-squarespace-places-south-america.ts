/**
 * Migrate Squarespace HTTrack South America place HTML to
 * `content/places/south-america/<country>/<place>.mdx` and copy images to
 * `migration/cdn-upload-ready/south-america/<country>/<place>/` for CDN upload
 * (public URLs: `https://media.togstrek.com/south-america/...` — same pattern as Tulum).
 *
 * Skips `tag/`, `category/`, and `_https_` embed stubs.
 *
 * Usage:
 *   npx tsx scripts/migrate-squarespace-places-south-america.ts --dry-run
 *   npx tsx scripts/migrate-squarespace-places-south-america.ts --backup /path/to/TogsTrekBackup
 *   npx tsx scripts/migrate-squarespace-places-south-america.ts --limit 3
 */
...
/** CDN paths mirror site URLs: /south-america/argentina/buenos-aires/photo.jpg */
...
function resetSouthAmericaStaging(cdnOut: string, dryRun: boolean): void {
  const target = path.join(cdnOut, "south-america");
  if (dryRun || !fs.existsSync(target)) return;
  fs.rmSync(target, { recursive: true });
}

/** Place posts: `south-america/<country>/<place>.html` only (no tag/category). */
function listSouthAmericaPlaceHtmlFiles(southAmericaDir: string): string[] {
  const out: string[] = [];
  let top: fs.Dirent[];
  try {
    top = fs.readdirSync(southAmericaDir, { withFileTypes: true });
  } catch {
    return out;
  }
  for (const country of top) {
    ...
    if (
      ...
      country.name === "_https_"
    ) {
      continue;
    }
    const countryDir = path.join(southAmericaDir, country.name);
    let inner: fs.Dirent[];
    try {
      inner = fs.readdirSync(countryDir, { withFileTypes: true });
    } catch {
      continue;
    }
    for (const ent of inner) {
      ...
      out.push(path.join(countryDir, ent.name));
    }
  }
  return out.sort((a, b) => a.localeCompare(b));
}

function htmlPathToSlugSegments(togstrekCom: string, htmlAbs: string): string[] {
...
function main(): void {
  ...
  const southAmericaDir = path.join(togstrekCom, "south-america");
  const contentRoot = path.join(REPO_ROOT, "content");
  const cdnOut = path.join(REPO_ROOT, "migration", "cdn-upload-ready");

  if (!fs.existsSync(southAmericaDir)) {
    console.error(`South America dir missing: ${southAmericaDir}`);
    process.exit(1);
  }
  ...
  resetSouthAmericaStaging(cdnOut, dryRun);
  if (!dryRun) {
    console.log(
      "Cleared migration/cdn-upload-ready/south-america for a clean place image tree.",
    );
  }

  let postHtml = listSouthAmericaPlaceHtmlFiles(southAmericaDir);
  if (limit !== null) postHtml = postHtml.slice(0, limit);

  console.log(
    `Migrating ${postHtml.length} South America place pages${limit ? ` (limit ${limit})` : ""}.`,
  );

  for (const htmlAbs of postHtml) {
    const slugSegments = htmlPathToSlugSegments(togstrekCom, htmlAbs);
    if (slugSegments.length !== 3 || slugSegments[0] !== "south-america") {
      console.warn(`Skip unexpected path: ${slugSegments.join("/")}`);
      continue;
    }
    migrateOnePlacePage({
      htmlAbs,
  ...
  if (dryRun) {
    ...
    console.log("Done.");
    console.log(
      `Upload ${pathToFileURL(path.join(cdnOut, "south-america")).href} to CDN with keys south-america/... (same as site paths).`,
    );
  }
}

 ...
 *   npx tsx scripts/migrate-squarespace-places-north-america.ts --backup /path/to/TogsTrekBackup
 *   npx tsx scripts/migrate-squarespace-places-north-america.ts --limit 3
 *
 ...
 *   (keys become north-america/mexico/tulum/… matching MDX URLs.)
 */
...
/** CDN paths mirror site URLs: /north-america/mexico/tulum/photo.jpg */
...
const NORTH_AMERICA_SKIP_DIRS = new Set(["tag", "category", "_https_"]);

function resetNorthAmericaStaging(cdnOut: string, dryRun: boolean): void {
  const target = path.join(cdnOut, "north-america");
  if (dryRun || !fs.existsSync(target)) return;
  fs.rmSync(target, { recursive: true });
}

/** All place `.html` files under `north-america/` (recursive), excluding tag/category. */
function listNorthAmericaPlaceHtmlFiles(northAmericaDir: string): string[] {
  const out: string[] = [];
  function walk(dir: string): void {
    ...
    for (const ent of entries) {
      const full = path.join(dir, ent.name);
      if (ent.isDirectory()) {
        if (NORTH_AMERICA_SKIP_DIRS.has(ent.name)) continue;
        walk(full);
      } else if (
        ent.isFile() &&
...
function htmlPathToNorthAmericaSlugSegments(
  ...
  const rel = path.relative(northAmericaDir, htmlAbs).replace(/\\/g, "/");
...
function main(): void {
  ...
  const northAmericaDir = path.join(togstrekCom, "north-america");
  const contentRoot = path.join(REPO_ROOT, "content");
  const cdnOut = path.join(REPO_ROOT, "migration", "cdn-upload-ready");

  if (!fs.existsSync(northAmericaDir)) {
    console.error(`North America dir missing: ${northAmericaDir}`);
    process.exit(1);
  }
  ...
  resetNorthAmericaStaging(cdnOut, dryRun);
  if (!dryRun) {
    console.log(
      "Cleared migration/cdn-upload-ready/north-america for a clean place image tree.",
    );
  }

  let postHtml = listNorthAmericaPlaceHtmlFiles(northAmericaDir);
  if (limit !== null) postHtml = postHtml.slice(0, limit);

  console.log(
    `Migrating ${postHtml.length} North America place pages${limit ? ` (limit ${limit})` : ""}.`,
  );

  for (const htmlAbs of postHtml) {
    const slugSegments = htmlPathToNorthAmericaSlugSegments(
      northAmericaDir,
      htmlAbs,
    );
  ...
  if (dryRun) {
    ...
    console.log("Done.");
    console.log(
      `Upload to CDN: npm run r2:upload -- --source migration/cdn-upload-ready/north-america --prefix north-america/`,
    );
    console.log(pathToFileURL(path.join(cdnOut, "north-america")).href);
  }
}

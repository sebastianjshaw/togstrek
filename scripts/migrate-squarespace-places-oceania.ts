/**
 * Migrate Squarespace HTTrack Oceania place HTML to
 * `content/places/oceania/<country>/…/<place>.mdx` (nested paths when the mirror
 * has subfolders, e.g. `australia/victoria/ballarat.html`) and copy images to
 * `migration/cdn-upload-ready/oceania/...` for CDN upload
 * (public URLs: `https://media.togstrek.com/oceania/...`).
 *
 * Backup layout: `togstrek.com/oceania/<country>/<place>.html` (recursive).
 * Skips `tag/`, `category/`, `_https_`, and `template-*.html`.
 *
 * Usage:
 *   npx tsx scripts/migrate-squarespace-places-oceania.ts --dry-run
 *   npx tsx scripts/migrate-squarespace-places-oceania.ts --backup /path/to/TogsTrekBackup
 *   npx tsx scripts/migrate-squarespace-places-oceania.ts --limit 3
 *
 ...
 *   npm run r2:upload -- --source migration/cdn-upload-ready/oceania --prefix oceania/
 *   (keys become oceania/australia/ballarat/… matching MDX URLs.)
 *
 * If the HTTrack backup has almost no image binaries (common), run after migrate:
 *   npm run media:fetch-oceania
 * That downloads Squarespace CDN URLs from the new MDX, writes files under
 * `migration/cdn-upload-ready/oceania/`, and rewrites MDX to `media.togstrek.com` paths.
 */
...
/** CDN paths mirror site URLs: /oceania/australia/ballarat/photo.jpg */
...
function migrateOnePlacePage(args: {
  ...
  if (args.slugSegments.length < 3 || args.slugSegments[0] !== "oceania") {
    console.warn(
      `Skip (expected oceania/<country>/…): ${args.slugSegments.join("/")}`,
    );
    return;
  }
...
const OCEANIA_SKIP_DIRS = new Set(["tag", "category", "_https_"]);

function resetOceaniaStaging(cdnOut: string, dryRun: boolean): void {
  const target = path.join(cdnOut, "oceania");
  if (dryRun || !fs.existsSync(target)) return;
  fs.rmSync(target, { recursive: true });
}

/** All place `.html` files under `oceania/` (recursive), excluding tag/category and templates. */
function listOceaniaPlaceHtmlFiles(oceaniaDir: string): string[] {
  const out: string[] = [];
  function walk(dir: string): void {
    ...
    for (const ent of entries) {
      const full = path.join(dir, ent.name);
      if (ent.isDirectory()) {
        if (OCEANIA_SKIP_DIRS.has(ent.name)) continue;
        walk(full);
      } else if (
        ent.isFile() &&
...
/**
 * Map backup file under `togstrek.com/oceania/` to full URL segments
 * `oceania/<country>/…/<place>` (e.g. nested `tanzania/zanzibar/stone-town`).
 */
function htmlPathToOceaniaSlugSegments(
  oceaniaDir: string,
  htmlAbs: string,
): string[] | null {
  const rel = path.relative(oceaniaDir, htmlAbs).replace(/\\/g, "/");
...
function main(): void {
  ...
  resetOceaniaStaging(cdnOut, dryRun);
  if (!dryRun) {
    console.log(
      "Cleared migration/cdn-upload-ready/oceania for a clean place image tree.",
    );
  }

  let postHtml = listOceaniaPlaceHtmlFiles(oceaniaDir);
  if (limit !== null) postHtml = postHtml.slice(0, limit);

  console.log(
    `Migrating ${postHtml.length} Oceania place pages${limit ? ` (limit ${limit})` : ""}.`,
  );

  for (const htmlAbs of postHtml) {
    const slugSegments = htmlPathToOceaniaSlugSegments(oceaniaDir, htmlAbs);
    if (!slugSegments || slugSegments[0] !== "oceania") {
      console.warn(
        `Skip unexpected path: ${posix(path.relative(oceaniaDir, htmlAbs))}`,
      );
      continue;
    }
    migrateOnePlacePage({
  ...
  if (dryRun) {
    ...
    console.log("Done.");
    console.log(
      `Upload to CDN: npm run r2:upload -- --source migration/cdn-upload-ready/oceania --prefix oceania/`,
    );
    console.log(pathToFileURL(path.join(cdnOut, "oceania")).href);
  }
}

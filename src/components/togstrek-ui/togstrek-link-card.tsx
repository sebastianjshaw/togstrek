...
const togstrekLinkCardCompactBaseClass =
  "togstrek-link-card togstrek-link-card--compact flex overflow-hidden rounded-[var(--tt-radius-photo)] border font-tt-body transition-colors";
...
export function TogstrekLinkCard(props: TogstrekLinkCardProps) {
  ...
  return (
    <Link
      href={href}
      className={`togstrek-link-card togstrek-link-card--region group relative flex min-h-[var(--tt-region-card-min-height)] flex-col justify-end overflow-hidden rounded-[var(--tt-radius-photo)] border border-tt-border-muted bg-tt-surface-base shadow-[var(--tt-shadow-photo-tile)] transition-[transform,box-shadow,border-color] duration-[var(--tt-duration-normal)] ease-[var(--tt-ease-out)] after:pointer-events-none after:absolute after:inset-0 after:rounded-[inherit] after:border-[length:var(--tt-border-width-thick)] after:border-transparent after:transition-colors hover:-translate-y-1 hover:shadow-[var(--tt-shadow-photo-tile-hover)] hover:after:border-tt-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-tt-accent focus-visible:ring-offset-2 ${

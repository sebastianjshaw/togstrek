...
const TogstrekExploreMap = dynamic(
    ...
    import("@/components/togstrek-explore-map").then((m) => m.TogstrekExploreMap),
  {
    ...
    loading: () => (
      <div
        className="togstrek-country-hub-map-loading flex h-[min(40vh,20rem)] items-center justify-center rounded-[var(--tt-radius-sm)] border border-tt-border-muted bg-tt-surface-muted font-tt-body text-tt-text-secondary sm:h-[min(48vh,26rem)] lg:h-[min(56vh,35rem)]"
        role="status"
      >
        Loading map…
      </div>
...
export function TogstrekCountryHubMap({
  ...
  if (places.length === 0) {
    return (
      <div
        className={`togstrek-country-hub-map-empty rounded-[var(--tt-radius-sm)] border border-tt-border-muted bg-tt-surface-muted px-6 py-12 text-center font-tt-body text-tt-text-secondary ${className}`}
        role="status"
      >

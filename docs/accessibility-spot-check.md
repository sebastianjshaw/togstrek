...
3. **Mega menu** — Open a desktop mega panel via hover/focus; **Escape** closes it and returns focus to the trigger. While the exit animation runs, the panel is `aria-hidden` + `inert` (not in the tab order). Triggers expose `aria-expanded` and `aria-controls` pointing at `#togstrek-site-header-mega-panel`.
